...
class ErrorTemplateViewSpec extends ViewSpecBase {
  ...
  private trait Test {
    ...
    val html = createView("error.unauthorized401.pageTitle.service", "error.unauthorized401.heading.service", "error.unauthorized401.message")
  ...
  "Error template view" should {
    ...
    "Display the correct page title in the browser for an unauthorized user" in new Test {
      Option(doc.getElementsByTag("title").first).fold(fail("Missing title")) { title =>
        title.text shouldBe s"${messages("error.unauthorized401.pageTitle.service")}${titleSuffix()}"
      }
    }

    "Display the correct page heading for an unauthorised user" in new Test {

      assertPageHeadingEqualsMessage(doc, "error.unauthorized401.heading.service")
    }
