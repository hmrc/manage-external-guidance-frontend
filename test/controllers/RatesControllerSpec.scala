...
class RatesControllerSpec extends ControllerBaseSpec with GuiceOneAppPerSuite with MockRatesService {
  ...
  private trait Test {
    ...
    val timescaleDetails: LabelledDataUpdateStatus = LabelledDataUpdateStatus(rates.size, Some(updateDetail))
  ...
  "GET /external-guidance/rates" should {

    "return OK and last update response" in new Test {

      MockRatesService.details().returns(Future.successful(Right(timescaleDetails)))
    ...
    "return BadRequest if receipt from server generates a validation error" in new Test {
      ...
      MockRatesService.details().returns(Future.successful(Left(ValidationError)))
    ...
    "return Unauthorized if receipt from server generates a forbidden error" in new Test {
      ...
      MockRatesService.details().returns(Future.successful(Left(ForbiddenError)))
    ...
    "return Internal error if receipt from server generates a NotFound error" in new Test {
      ...
      MockRatesService.details().returns(Future.successful(Left(NotFoundError)))
  ...
  "POST /external-guidance/rates" should {
    "Upload valid timescale defns return OK and last update response" in new Test {
      val fakeUploadRequest: FakeRequest[MultipartFormData[TemporaryFile]] = FakeRequest("POST", "/external-guidance/rates").withBody(createMultipartFormData(timscalesJsonString, "application/json"))

      MockRatesService.submitRates(ratesJson).returns(Future.successful(Right(timescaleDetails)))
    ...
    "Upload valid timescale defns but submission fails, return BAD_REQUEST and last update response" in new Test {
      ...
      MockRatesService.submitRates(ratesJson).returns(Future.successful(Left(BadRequestError)))
      MockRatesService.details().returns(Future.successful(Right(timescaleDetails)))
      val result: Future[Result] = controller.upload()(fakeUploadRequest)
    ...
    "Upload invalid timescale defns return BadRequest and last update response" in new Test {
      val fakeUploadRequest: FakeRequest[MultipartFormData[TemporaryFile]] = FakeRequest("POST", "/external-guidance/rates").withBody(createMultipartFormData("some text", "application/json"))
    ...
    "Upload valid timescale defns with non json content-type should return BadRequest and last update response" in new Test {
      val fakeUploadRequest: FakeRequest[MultipartFormData[TemporaryFile]] = FakeRequest("POST", "/external-guidance/rates").withBody(createMultipartFormData(timscalesJsonString, "application/txt"))
