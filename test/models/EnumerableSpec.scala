...
class EnumerableSpec extends AnyWordSpec with Matchers with EitherValues with OptionValues with Enumerable.Implicits {
  ...
  ".reads" must {
    ...
    Foo.values.foreach { value =>
      s"bind correctly for: $value" in {
        Json.fromJson[Foo](JsString(value.toString)).asEither.toOption.value shouldEqual value
      }
    }

    "fail to bind for invalid values" in {
      Json.fromJson[Foo](JsString("invalid")).asEither.left.value should contain(JsPath -> Seq(JsonValidationError("error.invalid")))
    }
  }
  ...
  ".writes" must {
    ...
    Foo.values.foreach { value =>
      s"write $value" in {
        Json.toJson(value) shouldEqual JsString(value.toString)
      }
    }
  }
